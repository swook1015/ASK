...
WINDOW_SIZE = 60
FALL_THRESHOLD = 0.9 # 최적의 기준점 0.9 적용!
...
for v_path in video_files:
    ...
    padded_lost = [is_lost_list[0]] * (WINDOW_SIZE - 1) + is_lost_list + [is_lost_list[-1]] * WINDOW_SIZE
    ...
    # [STEP 3] 확장된 시퀀스로 추론 및 뼈대 시각화 저장
    for idx, (kpts, frame, is_lost) in enumerate(zip(padded_kpts, padded_frames, padded_lost)):
        ...
        if len(kpts_buffer) == WINDOW_SIZE:
            ...
            ln_int.set_tensor(ln_in['index'], input_2d_q); ln_int.invoke()
            ...
            tcn_int.set_tensor(tcn_in['index'], tcn_in_q); tcn_int.invoke()
        ...
        # ==================================================
        # 💡 [영상 상단 상태창(블랙 바) 및 뼈대 그리기]
        # ==================================================
        ann_frame = frame.copy()
        
        # 1. 뼈대 그리기 (정상적으로 잡혔을 때만)
        if not is_lost and np.sum(kpts) > 0:
        ...
        # 🚨 2. 맨 위에 검은색 상태창(블랙 바) 꽉 차게 그리기! (높이 80px)
        cv2.rectangle(ann_frame, (0, 0), (width, 80), (0, 0, 0), -1)

        # 3. 상태별 텍스트 색상 및 문구 처리
        if current_frame_score > FALL_THRESHOLD:
            status = "FALL!!"
            color = (0, 0, 255) # 빨간색
        else:
            status = "LOST TARGET" if is_lost else "NORMAL"
            color = (128, 128, 128) if is_lost else (0, 255, 0) # 회색 / 초록색

        # 4. 텍스트를 검은색 바 안쪽 중앙에 예쁘게 배치 (y좌표 55)
        cv2.putText(ann_frame, f"STATUS: {status} | SCORE: {current_frame_score:.2f}", (20, 55), cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
