...
def residual_block(x, nodes, dropout_rate=0.5):
    ...
    # [단일 연산 세트] Dense + BN + ReLU + Dropout
    x = layers.Dense(nodes)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = layers.Dropout(dropout_rate)(x)
    
    # 2. 통과한 값과 원본 바로 더하기 (그림의 ⊕ 기호)
    return layers.Add()([shortcut, x])

def build_paper_lpn(input_dim=26, output_dim=39):
    inputs = layers.Input(shape=(input_dim,))
    
    # 1. 초기 진입층 (그림의 첫 번째 블록, 여기는 ⊕ 기호가 없음)
    x = layers.Dense(1024)(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = layers.Dropout(0.5)(x)
    
    # 2. 첫 번째 ⊕ 더하기 사이클 (Residual Block 1)
    x = residual_block(x, 1024)
    
    # 3. 두 번째 ⊕ 더하기 사이클 (Residual Block 2)
    x = residual_block(x, 1024)
    
    # 4. 최종 출력층 (그림의 마지막 1024 -> 3J)
    outputs = layers.Dense(output_dim)(x)
    
    model = models.Model(inputs, outputs, name="LPN_True_Paper_Structure")
