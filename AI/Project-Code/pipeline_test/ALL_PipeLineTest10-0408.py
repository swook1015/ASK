...
# ==========================================
# ⚙️ 1. 설정 및 경로 (모델 이름 최신화 확인!)
# ==========================================
VIDEO_DIR = './AI/dataset/nturgb+d_rgb_C001'
OUTPUT_DIR = './output_debug'
os.makedirs(OUTPUT_DIR, exist_ok=True)

YOLO_MODEL_PATH = './AI/models/yolo/yolo8n-pose_full_integer_quant.tflite'
LN_MODEL_PATH   = './AI/models/ln/0408new_lpn_cv_60_legacy.tflite' # 방금 성공한 98.47mm 모델!
TCN_MODEL_PATH  = './AI/models/1d-tcn/0408tcn_8layer_pure_ntu.tflite' # 이것도 새로 뽑은 TFLite로 꼭 변경하세요!
...
# 🚨 수정됨: max_dist를 내부에서 구하지 않고 외부에서 고정값으로 받음
...
for v_path in video_files:
    ...
    # 🚨 [새로 추가된 핵심 로직: 영상 전체 기준 스케일 고정]
    # skeleton2npy.py 에서 했던 것과 똑같이 전체 영상의 max_dist를 구합니다.
    ...
    # [STEP 2] 영상 및 시퀀스 강제 확장 (논문 방식 패딩) 
    ...
    for idx, (kpts, frame) in enumerate(zip(padded_kpts, padded_frames)):
        ...
        if len(kpts_buffer) == WINDOW_SIZE:
            # 🚨 수정됨: 위에서 구한 global_max_dist를 함수에 넘겨줌
