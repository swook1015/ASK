...
def center_poses_2d_final(X_2d_window):
    ...
    # 🚨 [수정 완료] Y축 뒤집기 삭제 (모델과 방향 완벽 일치)
    
    # 🚨 [수정 완료] 양자화 기절 방지 마법의 스케일링 적용
    max_dist = np.max(np.abs(centered)) + 1e-6
    normalized = (centered / max_dist) * (0.9 / 480.0)
...
for v_path in video_files:
    ...
    while cap.isOpened():
        ...
        # 🚨 [수정 완료] YOLO conf 0.10으로 하향 (쓰러진 사람 안 놓치게)
        results = yolo_model.predict(frame, verbose=False, conf=0.10)
